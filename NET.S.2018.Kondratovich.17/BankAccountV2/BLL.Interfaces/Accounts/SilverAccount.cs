...
namespace BLL.Interfaces.Accounts
{
    ...
    {
        public SilverAccount(string id, string name, decimal balance, int bonusPoint, Status status) : base(id, name, balance, bonusPoint, status) { }
        ...
        public override BankAccountTypes Type { get; } = BankAccountTypes.Silver;
        protected override int CalculateBonusPoints(decimal amount) => (int)Math.Round((int)amount * 0.25);
    }
}
